...
def main():
    ...
    #Collect data in two columns
        ...
        col2.append(float((str(row[0]).split(':'))[1]))

    # ask user to enter sampling time
    time_sample = float(input("Enter the sample time (^t) :- "))
    print("Sample time set by user:- "), str(time_sample )

    for j in range(0,len(col2)) :
        col1.append(time_sample*j)
